...
class DespesaServiceTest {
     ...
     * Mockito.when(despesaRepository.findById(anyLong())).thenReturn(Optional.of(
     * despesa));
     * Mockito.when(despesaRepository.save(any(Despesa.class))).thenReturn(despesa);
     * 
     ...
     * Mockito.when(despesaRepository.findAllByStatusPagamento(any(StatusPagamento.
     * class)))
     * .thenReturn(Collections.singletonList(despesa));
     * 
     * List<Despesa> foundDespesas =
     * despesaService.findByStatus(StatusPagamento.PENDENTE);
     * 
     * assertNotNull(foundDespesas);
     * assertEquals(1, foundDespesas.size());
     * assertEquals(StatusPagamento.PENDENTE,
     * foundDespesas.get(0).getStatusPagamento());
     * }
     * 
     * @Test
     * void testFindByPagamentoPendente() {
     * Mockito.when(despesaRepository.findAllByStatusPagamento(StatusPagamento.
     * PENDENTE))
     * .thenReturn(Collections.singletonList(despesa));
     * 
     ...
     * assertEquals(StatusPagamento.PENDENTE,
     * pendingDespesas.get(0).getStatusPagamento());
     * }
     * 
     * @Test
     * void testFindByVencimento() {
     * LocalDate dataVencimento = LocalDate.now().plusDays(30);
     * Mockito.when(despesaRepository.findByVencimento(any(StatusPagamento.class),
     * any(LocalDate.class)))
     * .thenReturn(Collections.singletonList(despesa));
     * 
     * List<Despesa> foundByVencimento =
     * despesaService.findByVencimento(StatusPagamento.PENDENTE, dataVencimento);
     * 
     * assertNotNull(foundByVencimento);
     * assertEquals(1, foundByVencimento.size());
     * assertEquals(StatusPagamento.PENDENTE,
     * foundByVencimento.get(0).getStatusPagamento());
     * }
     * 
     * @Test
