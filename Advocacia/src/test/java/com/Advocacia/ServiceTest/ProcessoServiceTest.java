...
class ProcessoServiceTest {
     ...
     * processo.setPrazosImportantes(Collections.singletonList(LocalDate.now().
     * plusDays(10)));
     * }
     * 
     * @Test
     * void testSaveProcesso() {
     * Mockito.when(processoRepository.save(any(Processo.class))).thenReturn(
     * processo);
     * 
     ...
     * Mockito.when(processoRepository.findById(anyLong())).thenReturn(Optional.of(
     * processo));
     * Mockito.when(processoRepository.save(any(Processo.class))).thenReturn(
     * processo);
     * 
     ...
     * Mockito.when(processoRepository.findById(anyLong())).thenReturn(Optional.
     * empty());
     * 
     ...
     * 
     * List<Processo> foundProcessos =
     * processoService.findByNumeroProcesso("0001234-56.2024.8.00.0000");
     * 
     * assertNotNull(foundProcessos);
     * assertEquals(1, foundProcessos.size());
     * assertEquals("0001234-56.2024.8.00.0000",
     * foundProcessos.get(0).getNumeroProcesso());
     * }
     */
}
