...
class DocumentoControllerTest {
     ...
     * documentoController.handleFileUpload(files,
     * LocalDate.now(), "PENDENTE", "Teste de documento");
     * 
     * assertEquals(HttpStatus.OK, response.getStatusCode());
     * assertEquals("Arquivos enviados com sucesso!",
     ...
     * ResponseEntity<Map<String, String>> response =
     * documentoController.handleFileUpload(
     ...
     * doThrow(new
     * IOException("Erro ao salvar o arquivo")).when(files[0]).transferTo(any(File.
     * class));
     * 
     * ResponseEntity<Map<String, String>> response =
     * documentoController.handleFileUpload(
     ...
     * assertEquals("Falha ao enviar o arquivo: Erro ao salvar o arquivo",
     ...
     * assertEquals("Por favor, selecione um arquivo para enviar.",
     * response.getBody().get("error"));
     * }
     * 
     * 
     ...
     * 
     * when(documentoService.update(eq(id),
     * any(Documento.class))).thenReturn(documentoAtualizado);
     * 
     * ResponseEntity<Documento> response = documentoController.update(id,
     * documentoAtualizado);
     * 
     * assertEquals(HttpStatus.OK, response.getStatusCode());
     ...
     * Documento documentoAtualizado = new Documento();
     * 
     * when(documentoService.update(eq(id), any(Documento.class))).thenThrow(new
