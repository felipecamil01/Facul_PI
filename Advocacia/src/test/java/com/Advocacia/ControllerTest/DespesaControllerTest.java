...
class DespesaControllerTest {
     ...
     * when(despesaService.update(anyLong(),
     * any(Despesa.class))).thenReturn(despesa);
     * 
     ...
     * when(despesaService.update(anyLong(),
     * any(Despesa.class))).thenThrow(EntityNotFoundException.class);
     * 
     * ResponseEntity<Despesa> response = despesaController.update(1L, despesa);
     * 
     ...
     * when(despesaService.findByStatus(any(StatusPagamento.class))).thenReturn(
     * Collections.singletonList(despesa));
     * 
     * ResponseEntity<List<Despesa>> response =
     * despesaController.findByStatus(StatusPagamento.PAGO);
     * 
     ...
     * when(despesaService.findByStatus(any(StatusPagamento.class))).thenThrow(new
     * RuntimeException());
     * 
     * ResponseEntity<List<Despesa>> response =
     * despesaController.findByStatus(StatusPagamento.PAGO);
     ...
     * when(despesaService.findByPagamentoPendente()).thenReturn(Collections.
     * singletonList(despesa));
     * 
     * ResponseEntity<List<Despesa>> response =
     * despesaController.findByPagamentoPendente();
     * 
     * assertEquals(HttpStatus.ACCEPTED, response.getStatusCode());
     * assertEquals(Collections.singletonList(despesa), response.getBody());
     * }
     ...
     * when(despesaService.findByPagamentoPendente()).thenThrow(new
     * RuntimeException());
     * 
     * ResponseEntity<List<Despesa>> response =
     * despesaController.findByPagamentoPendente();
     ...
     * when(despesaService.findByVencimento(any(StatusPagamento.class),
     * any(LocalDate.class)))
     * .thenReturn(Collections.singletonList(despesa));
     * 
     * ResponseEntity<List<Despesa>> response =
     * despesaController.findByVencimento(StatusPagamento.PAGO, LocalDate.now());
     * 
     * assertEquals(HttpStatus.ACCEPTED, response.getStatusCode());
     * assertEquals(Collections.singletonList(despesa), response.getBody());
     * }
     ...
     * List<String> categorias = Arrays.asList("Alimentação", "Saúde",
     * "Transporte");
     * 
