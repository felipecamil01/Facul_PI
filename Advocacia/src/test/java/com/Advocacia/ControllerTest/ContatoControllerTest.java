...
class ContatoControllerTest {
     ...
     * contatoDto.setId(1L);
     * 
     * when(contatoService.save(contatoDto)).thenThrow(new
     * EntityNotFoundException("Contato não encontrado"));
     * 
     * ResponseEntity<Contato> response = contatoController.save(contatoDto);
     * 
     ...
     * 
     * when(contatoService.update(eq(id),
     * eq(contatoDto))).thenReturn(contatoAtualizado);
     * 
     ...
     * ContatoDto contatoDto = new ContatoDto();
     * 
     * when(contatoService.update(eq(id), eq(contatoDto))).thenThrow(new
     ...
     * 
     * ResponseEntity<List<Contato>> response =
     * contatoController.findByIdCliente(clienteId);
     * 
     * assertEquals(HttpStatus.OK, response.getStatusCode());
